...
def aws_command(profile: str | None, *arguments: str) -> subprocess.CompletedProcess[str]:
...
def ensure_account_prerequisites(profile: str | None, configuration: Configuration) -> None:
    """Create account-shared AWS resources with the original AWS identity."""
    identity = aws_command(profile, "sts", "get-caller-identity", "--region", configuration.region)
    ...
    for role, service in (("AWSServiceRoleForECS", "ecs.amazonaws.com"), ("AWSServiceRoleForEC2Spot", "spot.amazonaws.com")):
        result = aws_command(profile, "iam", "get-role", "--role-name", role)
        if result.returncode == 0:
            continue
        if "(NoSuchEntity)" not in result.stderr:
            raise InstallError(f"Cannot inspect {role}: {result.stderr.strip()}")
        created = aws_command(profile, "iam", "create-service-linked-role", "--aws-service-name", service)
    ...
    if configuration.publisher_github_repositories:
        ...
        result = aws_command(profile, "iam", "get-open-id-connect-provider", "--open-id-connect-provider-arn", provider_arn)
        if result.returncode == 0:
            ...
            created = aws_command(
                profile, "iam", "create-open-id-connect-provider",
...
def execute(arguments: argparse.Namespace) -> None:
    ...
    ensure_account_prerequisites(arguments.profile, configuration)
