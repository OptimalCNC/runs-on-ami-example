...
After local publication, keep the newest available matching AMI and its one
backing snapshot by pruning older versions:

```sh
python3 -m publish.prune --config .local/publishing.json
```

The [publisher guide](publish/README.md#inputs) also covers deletion of an
individual publication. Follow its
[failure and recovery instructions](publish/README.md#failure-and-recovery)
if publication or cleanup fails or is interrupted. Keep publication records
while their AMIs or snapshots exist; deleting local files or letting GitHub
artifacts expire does not remove AWS resources.
...
Create the chosen GitHub environment in repository settings and configure its
allowed branches and any required approvals. The `publish` and `clean` jobs use
this environment to obtain temporary AWS credentials through OIDC. The publisher
role's trust policy requires an exact repository/environment match.

When using the RunsOn installer, `publishing.github_repositories[].repository`
must identify the repository running this workflow, and that entry's `environment`
must match the GitHub environment selected by the workflow's `environment` input.
Both default to `image-publish`, which scheduled runs also use through the job
environment fallbacks described below. The separate
`installation.environment` setting selects a RunsOn installation through the
runner's `env` label in execution and installation smoke workflows; it does not
select or authorize this publishing environment.

Configure **repository variables** under **Settings → Secrets and variables →
Actions → Variables**. These values apply to publishing across the repository;
keep them at repository scope:

| Repository variable | Meaning |
| --- | --- |
| `AWS_ACCOUNT_ID` | Required 12-digit AWS account ID that will own the image |
| `AWS_REGION` | Required AWS region where the image will be published |
| `IMAGE_TAGS` | Optional JSON object of caller tags; defaults to `{}`. Include all tags required by the publisher role. |
| `PUBLISHING_ROLE_ARN` | Required IAM role ARN authorized for GitHub OIDC |

The workflow assembles the publisher's [configuration](publish/README.md#inputs)
from `AWS_ACCOUNT_ID`, `AWS_REGION`, and `IMAGE_TAGS`, with the fixed image name
`ubuntu2404-xenomai-cobalt` for both publication and cleanup. The
[RunsOn installation guide](../runs-on/README.md#updates-exports-and-removal)
exports `account_id`, `region`, `required_tags`, and `publisher_role_arn` in its
publishing contract; use those for the corresponding variables.

For example, replacing the account and role with your values:
...
If the role requires tags, set `IMAGE_TAGS` to its required JSON key/value pairs.
For a RunsOn publisher role, copy the exported `required_tags` object:

```sh
gh variable set IMAGE_TAGS --body '{"runs-on-installation":"<installation.name>"}'
```
...
| Manual (`workflow_dispatch`) | Build and validate; publish and clean when `publish_image=true`. The `environment` input defaults to `image-publish`. |
| Schedule (`0 0 * * 1`) | Every Monday at 00:00 UTC, build, validate, publish, and clean from the default branch using `image-publish`. |
...
gh workflow run image-build.yml -f publish_image=true -f environment=image-publish
```

For a different interval, edit the workflow's `on.schedule` cron expression
(UTC). Scheduled runs use the `image-publish` fallback in both the `publish` and
