...
from example import (ROOT, BUILD_TAG, EXPIRY_TAG, OWNER_TAG, PURPOSE_TAG, RUNS_ON_TAG, Cloud, build_id,
                     filters_for, load_deployment, parse_time, require, resource_tags, tags_of, timestamp, write_json)
...
def registration_waiting(jobs, stock=False):
    targets = [("build", "configure")]
    if not stock:
...
def adopt_test_instances(cloud, build, jobs=None, *, image=None, expiry=None):
    ...
    for stage, job in (jobs or {}).items():
        ...
        require(instance["InstanceType"] == d.instance_type and instance.get("InstanceLifecycle", "on-demand") == "on-demand",
...
def monitor(cloud, variant, run_id, attempt, output, poll=15):
    d = cloud.deployment
    ...
    try:
        while True:
            ...
            report["jobs"] = current
            for instance in adopt_test_instances(cloud, build, current):
                report["observed_instances"][instance["InstanceId"]] = instance
            now = time.monotonic()
            require(now - started < d.deadlines["workflow_seconds"], "independent workflow deadline exceeded")
            for stage in registration_waiting(current, variant == "stock"):
            ...
            queued_since = {stage: since for stage, since in queued_since.items()
                            if stage in registration_waiting(current, variant == "stock")}
