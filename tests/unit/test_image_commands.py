        ...
        for build_id in (None, "789-2-stock"):
            with self.subTest(build_id=build_id):
                ...
                if build_id:
                    ...
                    self.assertIn("runs-on=789-2-stock-controller/", value["controller_label"])
                    self.assertIn("/ami=ami-0123456789abcdef0/", value["controller_label"])
                    self.assertTrue(value["controller_label"].endswith("/volume=30gb:gp3:125mbs:3000iops"))
                else:
