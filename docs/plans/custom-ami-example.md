...
Bootstrap foundation identities and the artifact store first, then install or import RunsOn, configure management access from the selected network, capture parents, and resolve the manifest. Keep plans, Terraform/CloudFormation state, accepted-image selections, and execution records in the deployment directory or private durable storage. A workflow authenticates through its protected environment and freezes the exact deployment-bundle version for each run attempt.
